...
Korjasinpa nyt tuonkin, vaikkei clienttiä moduulissa käytetäkkään.
 ...
 # Viimeinen testi puhtaalta pöydältä

aluksi poistin pyrin poistamaan kaiken mahdollisen

     $ sudo systemctl stop teamspeak.service 
     $ sudo systemctl disable teamspeak.service 
     Removed /etc/systemd/system/multi-user.target.wants/teamspeak.service.
     $ sudo rm /lib/systemd/system/teamspeak.service
     $ sudo rm -r /home/teamspeak
     $ sudo deluser teamspeak
     Removing user `teamspeak' ...
     Warning: group `teamspeak' has no more members.
     Done.
Varmistin vielä, ettei serveri kummittele jotenkin?

![deleted](./kuvat/deleted.PNG)

No ei servu tietenkään toiminut.

Sitten ajoin moduulin:

    $ sudo salt '*' state.apply teamspeak
aivan liikaa tekstiä, en pastea koko listaa, vaan alun ja lopun koska niiden tulokset ovat merkittäviä.
    ...
    Duration: 18.018 ms
     Changes:   
              ...
              stdout:
                  * teamspeak.service - TeamSpeak 3 Server
                     Loaded: loaded (/lib/systemd/system/teamspeak.service; enabled; vendor preset: enabled)
                     Active: active (running) since Wed 2020-12-16 23:51:19 EET; 365ms ago
                   Main PID: 15253 (ts3server)
                      Tasks: 15 (limit: 4666)
                     CGroup: /system.slice/teamspeak.service
                             `-15253 ./ts3server inifile=ts3server.ini daemon=1 pid_file=ts3server.pid
                  
                  Dec 16 23:51:19 noop ts3server_startscript.sh[15246]: Starting the TeamSpeak 3 server
                  Dec 16 23:51:19 noop ts3server_startscript.sh[15246]: TeamSpeak 3 server started, for details please view the log file
                  Dec 16 23:51:19 noop systemd[1]: Started TeamSpeak 3 Server.
                  Dec 16 23:51:19 noop ts3server_startscript.sh[15246]: ------------------------------------------------------------------
                  Dec 16 23:51:19 noop ts3server_startscript.sh[15246]:                       I M P O R T A N T
                  Dec 16 23:51:19 noop ts3server_startscript.sh[15246]: ------------------------------------------------------------------
                  Dec 16 23:51:19 noop ts3server_startscript.sh[15246]:                Server Query Admin Account created
                  Dec 16 23:51:19 noop ts3server_startscript.sh[15246]:          loginname= "serveradmin", password= "Ny3XDKzH"
                  Dec 16 23:51:19 noop ts3server_startscript.sh[15246]:          apikey= "BABY3m2DBvXnOI3cNA46qb9phCFAEExg85pFGfa"
                  Dec 16 23:51:19 noop ts3server_startscript.sh[15246]: ------------------------------------------------------------------
    ...
    ------------
    Total states run:     6
    Total run time:   2.287 s

Eli staten mukaan user oli vielä olemassa, vaikka ajoin sudo deluser ja poistin /home/teamspeak kotihakemiston. Ja systemctl status teamspeak näyttää eri tekstin kun aikaisemmin, nyt käytössä on apikey eikä token? Mistähän tämä muutos voi johtua.

Connectatessa kuitenkin clientillä, se pyytää privilege keytä.

![puhas](./kuvat/puhdastesticonnect.PNG)

Ajetaampa komento uudestaan. 
    ...
    Duration: 9.48 ms
     Changes:   
              ...
              stdout:
                  * teamspeak.service - TeamSpeak 3 Server
                     Loaded: loaded (/lib/systemd/system/teamspeak.service; enabled; vendor preset: enabled)
                     Active: active (running) since Wed 2020-12-16 23:51:19 EET; 7min ago
                   Main PID: 15253 (ts3server)
                      Tasks: 21 (limit: 4666)
                     CGroup: /system.slice/teamspeak.service
                             `-15253 ./ts3server inifile=ts3server.ini daemon=1 pid_file=ts3server.pid
                  ...
                  Dec 16 23:51:19 noop ts3server_startscript.sh[15246]:          apikey= "BABY3m2DBvXnOI3cNA46qb9phCFAEExg85pFGfa"
                  Dec 16 23:51:19 noop ts3server_startscript.sh[15246]: ------------------------------------------------------------------
                  Dec 16 23:51:21 noop ts3server_startscript.sh[15246]: ------------------------------------------------------------------
                  Dec 16 23:51:21 noop ts3server_startscript.sh[15246]:                       I M P O R T A N T
                  Dec 16 23:51:21 noop ts3server_startscript.sh[15246]: ------------------------------------------------------------------
                  Dec 16 23:51:21 noop ts3server_startscript.sh[15246]:       ServerAdmin privilege key created, please use it to gain
                  Dec 16 23:51:21 noop ts3server_startscript.sh[15246]:       serveradmin rights for your virtualserver. please
                  Dec 16 23:51:21 noop ts3server_startscript.sh[15246]:       also check the doc/privilegekey_guide.txt for details.
                  Dec 16 23:51:21 noop ts3server_startscript.sh[15246]:        token=8oZosDMOKNPRE4i0l2duhJi32G99uw6yx5ng+1Yi
                  Dec 16 23:51:21 noop ts3server_startscript.sh[15246]: ------------------------------------------------------------------

Ja kuinka oletinkaan, systemct status teamspeak tulos on taas eri, eli nyt löytyy privilege key, jota voin käyttää connectatessa saadakseni adminit käyttäjälle.

Teenpä vielä muutoksen init.sls tiedostoon, jotta cmd.run ajetaan vasta pienen viiveen jälkeen: jospa tämä selvittäisi asian.
...
testataan
    ...
    Function: module.run
     ...
     Started: 00:19:08.523425
    Duration: 3045.119 ms
     Changes:   
              ----------
              ret:
                  True
    ...
    Duration: 22.435 ms
     Changes:   
              ...
              stdout:
                  * teamspeak.service - TeamSpeak 3 Server
                     Loaded: loaded (/lib/systemd/system/teamspeak.service; enabled; vendor preset: enabled)
                     Active: active (running) since Wed 2020-12-16 23:51:19 EET; 27min ago
                   Main PID: 15253 (ts3server)
                      Tasks: 21 (limit: 4666)
                     CGroup: /system.slice/teamspeak.service
                             `-15253 ./ts3server inifile=ts3server.ini daemon=1 pid_file=ts3server.pid
...
Näyttää hyvältä tässä, testataan vielä moduulia tyhjältä pöydältä, ja koitan vielä poistaa käyttäjän saltilla, jolloin sen pitäisi varmasti poistua.
     ...
     $ cd /srv/salt/
     $ sudo mkdir deleteuser
     $ cd deleteuser/
     $ sudo init.sls
     teamspeak:
       user.absent 
     $ sudo salt '*' state.apply deleteuser
    ...
    Function: user.absent
      Result: True
     Comment: User teamspeak is not present
     Started: 00:24:59.226230
    Duration: 10.61 ms
     Changes:   
    ...
    ------------
    Total states run:     1
    Total run time:  10.610 ms

lisään vielä stateen purge: True ollakseni varma.
    ...
    ------------
    Total states run:     1
    Total run time:   6.574 ms

Nyt voin testata moduulini toimintaa toivottavasti viimeisen kerran.
...
Tästä voin olla niin ylpeä, että pasteen koko tilan. 

Viimeisenä vielä clientillä yhdistämisen testaus, ja nyt voidaan käyttää suoraan saamaamme tokenia saadaksemme admin oikeus.
...
# Hurraa! Koko moduulini toimii haluavamalla tavalla. 
loppuun vielä lähteet jota minulla on vielä auki:
